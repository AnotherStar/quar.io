...
quar.io — продукт на русском языке. Любой текст, который видит пользователь (UI, письма, ошибки, описания тарифов и модулей, пустые состояния, тултипы, подписи к полям) пишется по-русски. Английский допустим только там, где он технически необходим и нет устоявшегося русского эквивалента.

Обязательные правила:

- Не выставляй технические коды наружу. В UI должно быть «Модуль гарантии и регистрации», а не «Модуль warranty-registration»; «Чат-консультант», а не «chat-consultant». Машинные `code`-идентификаторы остаются в БД, registry и API, но не в пользовательском тексте.
- В UI запрещены служебные, временные и «честные внутренние» сообщения: упоминания тестовых режимов, имитаций оплаты, заглушек, «MVP», «временно», «следующим шагом подключим», ссылок на внутренние тикеты и интеграции в скобках. Пример того, как делать нельзя: «Сейчас работает имитация оплаты с фиксированной тестовой картой — настоящие списания не происходят. Реальный шлюз подключим следующим шагом». Информация о тестовом режиме — это дело логов, переменных окружения и админки, а не клиентского интерфейса.
- Тексты должны быть информативными и выверенными: без «рыбы», без шуток, без разговорных и сленговых формулировок («дёргается карта», «пуляется письмо», «по-человечески»). Тон — спокойный, деловой, конкретный. Без снисходительности и без маркетингового шума.
- Описывай поведение системы как факт, а не как процесс изнутри. Плохо: «Пополняйте счёт заранее — при продлении подписки оплата сначала списывается отсюда, и только если бонусов не хватает, дёргается ваша карта». Хорошо: «Баланс используется при продлении подписки в первую очередь. Если его недостаточно, остаток списывается с привязанной карты».
- Сообщения об ошибках объясняют, что произошло и что с этим делать. Без «Что-то пошло не так», без голых кодов и стек-трейсов в интерфейсе.
- Перед тем как закоммитить новый текст, перечитай его как пользователь, который видит продукт впервые. Если фраза звучит как реплика из чата разработчиков — переписывай.

Эти правила распространяются и на тексты, которые ИИ-агент генерирует сам (имена кнопок, заголовки, подсказки, описания, тосты), а не только на копирайт от человека.

---

## 7. Подключаемые модули инструкций
