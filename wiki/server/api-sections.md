...
Модуль `api-sections` реализует Nitro API routes для управления reusable/custom sections в ManualOnline. Он предоставляет CRUD-операции над секциями в рамках текущего tenant’а и используется дашбордом/редактором для создания, просмотра, обновления и удаления переиспользуемых блоков контента.
