...
Модуль `api-media` реализует Nitro API routes для загрузки медиафайлов в ManualOnline. Он поддерживает два сценария загрузки: прямую загрузку из браузера в S3/MinIO через presigned URL и fallback-загрузку через Nuxt-сервер в локальное/абстрактное хранилище. После успешной загрузки модуль создаёт запись `MediaAsset` в базе данных для учёта, очистки и контроля квот.
