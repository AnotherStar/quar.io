...
Модуль `api-billing` реализует Nitro API-роуты для получения состояния биллинга текущей компании (`tenant`) и активации пробного периода тарифа. В quar.io он используется серверной частью Nuxt 3 для управления тарифным состоянием, доступными возможностями плана и trial-сценарием для продавцов.
