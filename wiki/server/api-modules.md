...
Модуль `api-modules` реализует Nitro API routes для управления подключаемыми модулями ManualOnline и обработки публичных действий этих модулей. Он отвечает за выдачу списка доступных модулей tenant-а, включение/отключение модулей, а также за приём и просмотр заявок по модулям `feedback` и `warranty`.
