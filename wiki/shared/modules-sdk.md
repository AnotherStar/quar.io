...
`modules-sdk` задаёт контракты и реестр подключаемых модулей инструкций в ManualOnline. Модуль является единой точкой описания того, какие instruction modules доступны платформе, какие данные они декларируют в manifest и какие Vue-компоненты предоставляют для публичной страницы, дашборда и редактора.
