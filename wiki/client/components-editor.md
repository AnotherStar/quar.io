...
Модуль `components-editor` реализует клиентскую часть TipTap-редактора инструкций в quar.io. Он отвечает за WYSIWYG-редактирование контента инструкций, вставку медиа, таблиц, предупреждений, переиспользуемых секций, подключаемых модулей, колонок и служебных атрибутов блоков для аналитики и публичного рендера.
...
В модуле нет Nitro route-файлов, но Vue-компоненты обращаются к внутренним API quar.io:
