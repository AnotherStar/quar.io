...
Модуль `components-ui` содержит базовые переиспользуемые Vue-компоненты интерфейса из директории `components/ui`. Эти компоненты формируют визуальный фундамент ManualOnline: кнопки, карточки, бейджи, уведомления, поля ввода и строку для копирования/скачивания URL с QR-кодом.
...
Стили задаются через классы, предположительно основанные на Tailwind CSS и дизайн-системе ManualOnline:
...
- Компоненты жестко завязаны на наличие дизайн-токенов и utility-классов проекта. При использовании вне текущей темы ManualOnline потребуется перенос соответствующих стилей.
