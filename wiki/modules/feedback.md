...
Модуль `feedback` добавляет в публичную инструкцию форму обратной связи с полями ФИО, телефон, email, Telegram и сообщение. В quar.io он используется как подключаемый блок инструкции, позволяющий посетителю отправить обращение продавцу или владельцу инструкции.
...
Файл экспортирует `ModuleDefinition` для SDK подключаемых модулей quar.io.
