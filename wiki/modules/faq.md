...
Модуль `faq` добавляет в инструкцию интерактивный блок часто задаваемых вопросов в формате аккордеона. Он используется как подключаемый модуль инструкции quar.io и настраивается отдельно для каждого экземпляра блока прямо в TipTap-редакторе.
