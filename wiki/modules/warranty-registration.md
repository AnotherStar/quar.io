...
Модуль `warranty-registration` добавляет на публичную страницу инструкции форму регистрации расширенной гарантии. Он используется как подключаемый модуль quar.io и позволяет покупателю отправить данные о себе, покупке и товаре прямо из опубликованной инструкции.
...
Файл экспортирует объект `ModuleDefinition`, который описывает модуль для SDK подключаемых instruction-модулей quar.io.
...
Это означает, что подключение или использование модуля должно быть ограничено пользователями/проектами с тарифом `plus` или выше, если такая проверка реализована на стороне платформы quar.io.
