...
const COMMANDS: Cmd[] = [
  ...
  { id: 'toggle', title: 'Сворачиваемый блок', hint: 'Заголовок видно всегда, тело раскрывается по клику', icon: 'lucide:chevron-right', run: (e) => e.chain().focus().deleteRange(currentRange.value!).insertToggle().run() },
  ...
  { id: 'module-ref', title: 'Модуль', hint: 'Вставить модуль (гарантия, FAQ, отзывы)', icon: 'lucide:puzzle', run: (e) => e.chain().focus().deleteRange(currentRange.value!).insertModuleRef().run() },
