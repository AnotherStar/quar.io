...
// Плавающая кнопка чата поддержки. Показывается на всех публичных
// инструкциях тенанта, у которого включён модуль chat-consultant.
// Логика тикетов и сообщений совпадает с instruction-modules/chat-consultant/Public.vue,
// различие только в триггере: вместо карточки-блока — fixed bottom-right иконка.
...
<template>
  <ClientOnly>
    <button
      type="button"
      class="global-chat-launcher"
      :aria-label="open ? 'Закрыть чат поддержки' : 'Открыть чат поддержки'"
      :aria-expanded="open"
      @click="open = !open"
    >
      <Icon :name="open ? 'lucide:x' : 'lucide:message-circle'" class="h-6 w-6" />
    </button>

    <Teleport to="body">
      <div
        v-if="open"
        class="global-chat-modal z-[9999] flex items-end justify-center bg-black/25 p-0 sm:items-center sm:p-md"
        role="dialog"
        aria-modal="true"
        aria-label="Чат поддержки"
        @click.self="open = false"
      >
        <div class="global-chat-modal__panel flex w-full flex-col overflow-hidden bg-canvas shadow-modal sm:h-[min(720px,calc(100dvh-48px))] sm:max-w-[560px] sm:rounded-xl">
...
.global-chat-launcher:focus-visible {
